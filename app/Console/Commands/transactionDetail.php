...
class transactionDetail extends Command
{
     ...
     * @var string
     */
    protected $signature = 'shareasale:transactiondetail';
    ...
    public function handle() {
        $file = dirname(__FILE__) . '/transactiondetail.xml';
        $xml = file_get_contents($file);
        $xml = htmlspecialchars_decode($xml);

        $xml = str_replace('<lastreferer>', '<lastreferer><![CDATA[', $xml);
        $xml = str_replace('</lastreferer>', ']]></lastreferer>', $xml);
        $xml = str_replace('<bannername>', '<bannername><![CDATA[', $xml);
        $xml = str_replace('</bannername>', ']]></bannername>', $xml);
        $xml = str_replace('<bannerpage>', '<bannerpage><![CDATA[', $xml);
        $xml = str_replace('</bannerpage>', ']]></bannerpage>', $xml);
        $xml = str_replace('<useragent>', '<useragent><![CDATA[', $xml);
        $xml = str_replace('</useragent>', ']]></useragent>', $xml);
